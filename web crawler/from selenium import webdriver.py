...
def web_crawler():
    ...
    date.send_keys('1130723')  #Ex: 要預約113年8月15日, input'1130815'  


    place1 = Select(driver.find_element(By.ID, 'dmvNoLv1'))
    place1.select_by_index('7')  #index:{1~7} represents {臺北市區監理所（含金門馬祖）,臺北區監理所（北宜花）,新竹區監理所（桃竹苗）,臺中區監理所（中彰投）,嘉義區監理所（雲嘉南）,高雄市區監理所,高雄區監理所（高屏澎東）} respectively.


    place2 = Select(driver.find_element(By.ID, 'dmvNo'))
    place2.select_by_index('3')  #index depends on place1 u chose, u can check the index on website in advance.
    ...
    personal_id.send_keys('R222164993')  #input


    birth = driver.find_element(By.ID, 'birthdayStr')
    birth.send_keys('0590702') #Ex:生日為95年1月1日, input:0950101


    name = driver.find_element(By.ID, 'name')
    name.send_keys('王郁菁')  #input


    phone_no = driver.find_element(By.ID, 'contactTel')
    phone_no.send_keys('0935916980')  #input


    email = driver.find_element(By.ID, 'email')
    email.send_keys('cmk606@tn.edu.tw')  #input
